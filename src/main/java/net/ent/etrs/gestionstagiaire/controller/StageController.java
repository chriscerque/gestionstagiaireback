...
//        return stageRepo.findAll();
        try {
            return ResponseEntity.ok(serviceStage.findAll());
        } catch (Exception e) {
    ...
    public ResponseEntity<StageDto> create(@RequestBody @Valid StageDto stageDto) {
        ...
        Optional<StageDto> oStage = serviceStage.save(stageDto);
        if (oStage.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        return ResponseEntity.ok(oStage.get());
...
//        }
    ...
    public ResponseEntity<StageDto> upadte(@PathVariable("id") Long id, @RequestBody @Valid StageDto stageDto) {
        ...
        StageDto s = this.serviceStage.save(stageDto).orElseThrow();
...
//        }
    ...
    public ResponseEntity<StageDto> findById(@PathVariable("id") Long id) {
        try {
            Optional<StageDto> oStage = this.serviceStage.findById(id);
            if (oStage.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
            }
            return ResponseEntity.ok(oStage.get());
